...
export default async function LandingPage() {
  ...
  const { count: totalUsers } = await serviceClient.from("profiles").select("*", { count: "exact", head: true })
  const { count: totalCheckIns } = await serviceClient.from("check_ins").select("*", { count: "exact", head: true })
    ...
    <div className="min-h-svh bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="glass mb-12 border-0 p-4 shadow-2xl sm:p-6 md:p-8">
          <h2 className="mb-4 text-center text-lg font-bold text-gray-900 sm:text-xl md:text-2xl">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          <Card className="glass group border-0 p-4 transition-all hover:scale-105 hover:shadow-xl sm:p-6">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900 sm:text-3xl">{totalUsers || 0}</p>
                <p className="text-xs font-medium text-muted-foreground sm:text-sm">Soul Members</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="glass group border-0 p-4 transition-all hover:scale-105 hover:shadow-xl sm:p-6">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900 sm:text-3xl">{totalCheckIns || 0}</p>
                <p className="text-xs font-medium text-muted-foreground sm:text-sm">Emotions Logged</p>
              </div>
            </div>
          </Card>
